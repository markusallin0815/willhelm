  ...
  return (
    <main style={{ background: '#0F0704', minHeight: '100vh' }} className="px-6 py-16 md:px-16">
      <div className="max-w-2xl mx-auto">
        <Link href="/" className="text-accent-gold text-sm hover:underline mb-10 inline-block">{t.datenschutz.back}</Link>

        <h1 className="text-bg-white font-black text-4xl mb-10">{t.datenschutz.title}</h1>

        {t.datenschutz.sections.map((section, i) => {
          const s = section as { heading: string; content: string; email?: string };
          return (
            <section key={i} className={i < t.datenschutz.sections.length - 1 ? 'mb-8' : ''}>
